...
.cover-photo {
	height: 160px;
	background: linear-gradient(135deg, rgba(255, 51, 102, 0.8),
...
<body>
				...
				<div class="tab-content" id="profileTabContent">
					...
					<div
						...
						<form action="${pageContext.request.contextPath}/profile/update"
							method="POST"
							class="p-4 rounded-4 mb-4 border border-white shadow-sm"
						...
						<div class="mt-5">
							...
							<c:choose>
								...
								<c:when test="${empty cheatSheetsList}">
									<div
										class="text-center text-muted py-5 bg-light rounded-4 border border-dashed mb-4">
										<i
													...
													<div>
														<div
															...
															<h5 class="fw-bold m-0">
																<a
																	href="${pageContext.request.contextPath}/cheatsheet/detail/${sheet.obfuscatedId}"
																	class="text-dark text-decoration-none hover-underline fs-5">
																	${sheet.title} </a>
															</h5>
															...
															<c:choose>
																...
																<c:when test="${sheet.visibility == 'FRIEND-ONLY'}">
																	<span class="visibility-pill pill-friends"> <i
																		class="bi bi-people-fill"></i> Friends
																	</span>
																</c:when>
																<c:otherwise>
																	<span class="visibility-pill pill-private"> <i
																		class="bi bi-lock-fill"></i> Private
																	</span>
																</c:otherwise>
															</c:choose>
														</div>
														...
														<div class="card-meta-item">
															<i class="bi bi-folder-fill"></i> ${sheet.category.name}
															Cheat Sheets
														</div>
														<div class="card-meta-item">
															<i class="bi bi-calendar-plus"></i> Created:
															<fmt:formatDate value="${sheet.createdAt}"
																pattern="yyyy-MM-dd" />
														</div>
														<div class="card-meta-item">
															<i class="bi bi-calendar-event"></i> Updated:
															<fmt:formatDate value="${sheet.updatedAt}"
																pattern="yyyy-MM-dd" />
														</div>
														...
														<div class="d-flex flex-wrap gap-2 my-3">
															<c:forEach items="${sheet.tags}" var="tag">
																<a
																	href="${pageContext.request.contextPath}/cheatsheet/tag/${tag.id}"
																	class="tag-badge-link">#${tag.name}</a>
															</c:forEach>
														</div>
													</div>

													<div
														class="stats-section mt-auto d-flex justify-content-between align-items-center border-top pt-3">
														<div class="d-flex gap-3">
															<span><i class="bi bi-eye-fill me-1"></i>
																${sheet.viewCount != null ? sheet.viewCount : 0}</span> <span><i
																class="bi bi-download me-1"></i> ${sheet.downloadCount != null ? sheet.downloadCount : 0}</span>
														</div>
														<div>
															<a
																href="${pageContext.request.contextPath}/cheatsheet/view-pdf/${sheet.id}"
																target="_blank"
																...
																class="bi bi-file-earmark-pdf-fill"></i> PDF
															</a>
														</div>
													</div>
												</div>
							...
							<h5 class="fw-bold text-dark mb-3 pt-3 border-top">
								<i class="bi bi-share-fill me-2 text-success"></i>Shared Cheat
								Sheets
							</h5>
							...
							<c:choose>
								<c:when test="${empty sharedCheatSheetsList}">
									<div
										class="text-center text-muted py-4 bg-white rounded-3 border border-dashed mb-4">
											...
											<div class="col-md-6">
												<div class="cheatsheet-card border-success-subtle shadow-sm">
													<h6 class="fw-bold mb-1">
														<a
															href="${pageContext.request.contextPath}/cheatsheet/detail/${item.cheatsheet.obfuscatedId}"
															class="text-decoration-none text-dark"> <c:out
																value="${item.cheatsheet.title}" />
														</a>
													</h6>
													<p class="text-secondary small text-truncate mb-2">
														<c:out value="${item.cheatsheet.description}" />
													</p>
													...
													<div
														...
														</span> <small class="text-muted"><i
															class="bi bi-eye me-1"></i> ${item.cheatsheet.viewCount != null ? item.cheatsheet.viewCount : 0}</small>
													</div>
												</div>
											</div>
										</c:forEach>
					...
					<div
						...
						<div class="p-4 rounded-4 border border-white shadow-sm mt-2"
							style="background: rgba(255, 255, 255, 0.5);">
							...
							<form id="changePasswordForm"
								...
								<div class="mb-3">
									...
									<div id="oldPasswordError" class="error text-danger"
								...
								<div class="mb-3">
									...
									<div id="newPasswordError" class="error text-danger"
										style="display: none; font-size: 12px; margin-top: 5px;"></div>
								</div>
								...
								<div class="mb-4">
									...
									<div id="confirmNewPasswordError" class="error text-danger"
										style="display: none; font-size: 12px; margin-top: 5px;"></div>
								</div>
	...
	<script>
		(function() {
			...
			const oldPasswordError = document
					.getElementById("oldPasswordError");
			const newPasswordError = document
					.getElementById("newPasswordError");
			const confirmNewPasswordError = document
					.getElementById("confirmNewPasswordError");
