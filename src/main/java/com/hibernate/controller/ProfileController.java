...
public class ProfileController {
	...
	public String showProfilePage(HttpSession session, Model model) {
		...
		// ၃။ မိမိမှတစ်ဆင့် ထပ်ဆင့် Share ထားသော Cheat Sheets များကို
		// စစ်ထုတ်ဆွဲထုတ်ခြင်း
		// 🌟 ၃။ မိမိမှတစ်ဆင့် ထပ်ဆင့် Share ထားသော Cheat Sheets များကို Repository မှ
		// တိုက်ရိုက်ဆွဲထုတ်ခြင်း
		try {
			// 💡 Java Stream Filter နေရာတွင် ဤသို့ ပြောင်းလဲခေါ်ယူလိုက်ပါ
			List<SharedCheatsheetEntity> mySharedList = sharedCheatsheetRepository
					.findMySharedWithDetails(user.getId());
	...
	public String changePassword(
		...
		if (newPassword.equals(oldPassword)) {
			redirectAttributes.addFlashAttribute("errorMessage", "New password cannot be the same as old password.");
			return "redirect:/profile?tab=security";
		}
		...
		if (success) {
			...
			redirectAttributes.addAttribute("success", "password_changed");
			session.invalidate();
			return "redirect:/login";
		} else {

			redirectAttributes.addFlashAttribute("errorMessage", "Incorrect current password. Please try again.");
			return "redirect:/profile?tab=security";
		}
	}
	...
	public org.springframework.http.ResponseEntity<org.springframework.core.io.Resource> getAvatar(
		...
		try {
			java.nio.file.Path filePath = java.nio.file.Paths.get("C:/my_project_uploads/").resolve(filename)
					.normalize();
			...
			if (resource.exists()) {
				...
				return org.springframework.http.ResponseEntity.ok()
						.contentType(org.springframework.http.MediaType.parseMediaType(contentType)).body(resource);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
